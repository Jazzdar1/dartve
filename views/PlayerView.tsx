...
interface PlayerViewProps {
  match: Match | null;
  onBack: () => void;
  relatedChannels: Channel[]; // In the parent, pass ONLY playlist channels if playing a playlist
  onSelectRelated: (channel: Channel) => void;
  isPlaylistMode?: boolean; // NEW: Tells the player if we are strictly in a playlist
}
...
const PlayerView: React.FC<PlayerViewProps> = ({ match, onBack, relatedChannels, onSelectRelated, isPlaylistMode = false }) => {
  ...
  // 🔥 NEW: Vercel/HTTPS CORS Proxy State
  ...
  const handleFallback = useCallback(() => {
    setPlayerEngine(prev => {
      ...
      if (idx < engines.length - 1) {
        ...
        setError("Origin Blocked by HTTPS/CORS! Turn on 'Vercel Proxy' in settings or use External Player.");
        setLoading(false);
        return prev;
      }
    });
    ...
    <div className="flex flex-col h-screen bg-[#0f1115] overflow-hidden text-white">
      ...
      {!isFullscreen && (
        <div className="flex-1 overflow-y-auto bg-[#0f1115] pb-24">
          ...
          <div className="p-4">
            {/* 🔥 NEW: Dynamic title based on Playlist Mode */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {relatedChannels.map((channel, idx) => (
                <button key={idx} onClick={() => onSelectRelated(channel)} className={`flex items-center gap-4 p-3 border rounded-xl transition-all text-left ${match.id === channel.id ? 'bg-[#00b865]/10 border-[#00b865]/50' : 'bg-[#1a1d23] border-white/5 hover:border-white/20'}`}>
                  <img src={channel.logo} loading="lazy" className="w-10 h-10 rounded-lg object-contain bg-white/5" onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = `https://ui-avatars.com/api/?name=TV` }} />
